...
def nt_remainder6():
    p = [
        P_mc("ntr6_b0", "17 除以 5,余数是?", [2, 3, 1, 4], 0, "17=5×3+2", "余 2", B),
        P_mc("ntr6_b1", "下面哪个数除以 4 余 1?", [9, 8, 12, 16], 0, "试着减 1 后能否被 4 整除", "9−1=8,能被 4 整除", B),
        P_mc("ntr6_b2", "今天星期一,100 天后是星期几?", ["星期三", "星期二", "星期一", "星期四"], 0, "100÷7=14 余 2", "一往后数 2 天是三", B),
        P_mc("ntr6_c0", "10 个 2 连乘(2×2×…×2),个位数字是?", [4, 2, 8, 6], 0, "2 的幂个位按 2,4,8,6 循环", "10÷4 余 2,对应 4", C),
        P_mc("ntr6_c1", "1,2,3,1,2,3,… 第 50 个数是?", [2, 1, 3, 50], 0, "每 3 个循环,50÷3 余 2", "第 2 个是 2", C),
        P_mc("ntr6_c2", "一个数除以 3 余 2、除以 5 余 2,最小的两位数是?", [17, 32, 12, 22], 0, "除 3 和除 5 都余 2 → 除 15 余 2", "15+2=17", C),
        P_mc("ntr6_c3", "100 除以 7,余数是?", [2, 3, 4, 1], 0, "7×14=98", "100−98=2", C),
        P_mc("ntr6_x0", "3 的 20 次方,个位数字是?", [1, 3, 9, 7], 0, "3 的幂个位按 3,9,7,1 循环", "20÷4 余 0,对应第 4 个 1", X),
        P_mc("ntr6_x1", "现在 8 点,经过 100 小时后是几点?", ["12点", "8点", "4点", "0点"], 0, "100÷24 余 4", "8+4=12 点", X),
        P_mc("ntr6_x2", "珠子按 红黄蓝绿 四色循环穿,第 2024 个是什么颜色?", ["绿", "红", "黄", "蓝"], 0, "2024÷4 余 0,对应第 4 个", "绿色", X),
    ]
    return make_set("nt_remainder6", "余数与周期问题", "logic", p)


def comb_casework6():
    p = [
        P_mc("cw6_b0", "用 1,2,3 组成三位数(数字不重复),有几个?", [6, 9, 3, 27], 0, "3×2×1", "6 个", B),
        P_mc("cw6_b1", "投两个骰子,点数和为 4 的情况有几种?", [3, 2, 4, 6], 0, "(1,3)(2,2)(3,1)", "3 种", B),
        P_mc("cw6_b2", "从 4 个人中选 2 人握手,共几次?", [6, 4, 8, 12], 0, "4×3÷2", "6 次", B),
        P_mc("cw6_c0", "用 5 角和 1 元硬币凑成 2 元,有几种凑法?", [3, 2, 4, 5], 0, "按 1 元的张数分类:0/1/2 张", "3 种", C),
        P_mc("cw6_c1", "两位数中,十位与个位数字之和为 5 的有几个?", [5, 4, 6, 9], 0, "十位从 1 到 5", "14,23,32,41,50 共 5 个", C),
        P_mc("cw6_c2", "从 1~9 中选 2 个不同数字,和为 10 的有几组?", [4, 5, 3, 8], 0, "配对找", "1+9,2+8,3+7,4+6 共 4 组", C),
        P_mc("cw6_c3", "投两个骰子,点数和为 7 的有几种?", [6, 5, 4, 7], 0, "从 (1,6) 数到 (6,1)", "6 种", C),
        P_mc("cw6_x0", "用 0,1,2,3 组成没有重复数字的三位数(首位不为 0),有几个?", [18, 24, 12, 9], 0, "总数减去 0 在首位的", "24−6=18", X),
        P_mc("cw6_x1", "3 个相同的苹果分给 2 个小朋友(可以有人分到 0 个),有几种分法?", [4, 3, 6, 2], 0, "按第一人分到的个数分类", "(0,3)(1,2)(2,1)(3,0) 共 4 种", X),
        P_mc("cw6_x2", "平面上 5 个点(任意三点不在一条线上),最多能连几条线段?", [10, 20, 5, 25], 0, "每两点连一条,5×4÷2", "10 条", X),
    ]
    return make_set("comb_casework6", "分类枚举计数", "logic", p)


def geo_circle6():
    p = [
        P_mc("gc6_b0", "半径 2 的圆,直径是?", [4, 2, 1, 8], 0, "直径 = 半径 ×2", "2×2=4", B),
        P_mc("gc6_b1", "直径 10 的圆,半径是?", [5, 10, 20, 2], 0, "半径 = 直径 ÷2", "10÷2=5", B),
        P_mc("gc6_b2", "半圆的面积是同半径整圆面积的几分之几?", ["1/2", "1/4", "1/3", "2/3"], 0, "半圆是圆的一半", "1/2", B),
        P_mc("gc6_c0", "半径 3 的圆,周长是?(π取 3.14)", [18.84, 9.42, 28.26, 6.28], 0, "周长 = 2πr", "2×3.14×3=18.84", C),
        P_mc("gc6_c1", "半径 2 的圆,面积是?(π取 3.14)", [12.56, 6.28, 4, 25.12], 0, "面积 = πr²", "3.14×2×2=12.56", C),
        P_mc("gc6_c2", "半径 10 的圆,面积是?(π取 3.14)", [314, 31.4, 62.8, 100], 0, "面积 = πr²", "3.14×100=314", C),
        P_mc("gc6_c3", "一个圆的半径扩大到原来的 2 倍,面积变为原来的几倍?", [4, 2, 8, 3], 0, "面积和半径的平方有关", "2×2=4 倍", C),
        P_mc("gc6_x0", "边长 4 的正方形里画一个最大的圆,圆的面积是?(π取 3.14)", [12.56, 16, 50.24, 6.28], 0, "最大圆直径 = 边长 4,半径 2", "3.14×2×2=12.56", X),
        P_mc("gc6_x1", "边长 4 的正方形里画最大的圆,正方形内圆外的阴影面积是?(π取 3.14)", [3.44, 4, 12.56, 8], 0, "正方形面积减圆面积", "16−12.56=3.44", X),
        P_mc("gc6_x2", "直径 8 的半圆,周长(含直径)是?(π取 3.14)", [20.56, 12.56, 25.12, 16], 0, "半圆弧长 + 直径", "3.14×8÷2+8=20.56", X),
    ]
    return make_set("geo_circle6", "圆与阴影面积", "logic", p)
...
UNITS_G6 = [
    ...
    ("compnt", "lower", 100, "竞赛拓展·数论与组合", [
        ("nt_remainder6",          "余数与周期问题",      "logic",     []),
        ("comb_casework6",         "分类枚举计数",        "logic",     []),
    ]),
    ("compgeo", "lower", 101, "竞赛拓展·几何", [
        ("geo_circle6",            "圆与阴影面积",        "logic",     []),
    ]),
]
